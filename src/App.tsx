...
export default function App() {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      const userData: UserProfile = { uid, phone, password, displayName, role: 'user', shortId, createdAt: Date.now(), isActiveUntil: null };
  ...
  const handleGenerate = async () => {
    ...
    const now = Date.now();
    if (profile?.role !== 'admin' && (!profile?.isActiveUntil || profile.isActiveUntil < now)) {
      return setError('اشتراكك خلص يا نجم، كلم المدير فعل الحساب');
    }
...
افصل بين كل بوست والتاني بكلمة "---SPLIT---".`;
    ...
    try {
      ...
      if (text) {
        ...
        await updateDoc(doc(db, 'stats', 'global'), { aiRequestsCount: increment(1) }).catch(()=>{});
      } else {
  ...
  const generateKey = async (days: number) => {
    ...
    await setDoc(doc(db, 'activationKeys', id), { days, used: false, createdAt: Date.now() });
  };
      ...
      <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-blue-600 to-indigo-900 font-cairo" dir="rtl">
        ...
        >
          <div className="p-8 text-center bg-blue-50">
            ...
            <p className="text-blue-600 font-medium">كاتب البوستات المصري الروش</p>
          </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    {[7, 30, 90, 365].map(days => (
                      <button 
                        key={days}
                        onClick={() => generateKey(days)}
                        className="p-4 bg-gray-50 border-2 border-transparent hover:border-blue-500 rounded-2xl font-bold text-sm transition-all"
                      >
                        {days === 365 ? 'سنة' : `${days} يوم`}
                      </button>
                    ))}
                  </div>
                </div>
